...
# - ``EXTRA_ARGS`` (input): extra arguments, which will be passed to immortalc as ``--extra-arg=<string>``options.
# - ``INSTRUMENTABLE_DIRECTORIES`` (input): instrumentable header directories
# - ``IGNORE_SOURCES`` (input): sources to be ignored
# - ``EXTRA_IMMORTALC_ARGS`` (input): extra arguments for immortalc
function(_IMMORTAL_ADD_INSTRUMENTED_SOURCE_FILES _TARGET
		_NEW_TARGET _INFIX _EXTRA_ARGS _INSTRUMENTABLE_DIRECTORIES
		_IGNORE_SOURCES _EXTRA_IMMORTALC_ARGS)
...
#   to conflict.
# - ``OPTIMIZATION`` (input): type of optimization. See ``immortalc --help``,
...
function(ADD_IMMORTAL_LIBRARY _LIBRARY_TARGET)
	cmake_parse_arguments(PARSE_ARGV 1
		...
		"KEEP_ORIGINAL_TARGET_IN_ALL"
		"INFIX;OPTIMIZE"
		"EXTRA_ARGS;INSTRUMENTABLE_DIRECTORIES;IGNORE_SOURCES")
	...
	set(EXTRA_IMMORTALC_ARGS "")
	if (_FUNC_NAMED_PARAMETERS_OPTIMIZE)
		list(APPEND EXTRA_IMMORTALC_ARGS --optimize ${_FUNC_NAMED_PARAMETERS_OPTIMIZE})
	endif()

	set(TARGET_NAME ${_LIBRARY_TARGET}_immortal)
	add_library(${TARGET_NAME} STATIC "")
	_IMMORTAL_ADD_INSTRUMENTED_SOURCE_FILES(
	${_LIBRARY_TARGET}
	${TARGET_NAME}
	"${_FUNC_NAMED_PARAMETERS_INFIX}"
	${_FUNC_NAMED_PARAMETERS_EXTRA_ARGS}
	"${_FUNC_NAMED_PARAMETERS_INSTRUMENTABLE_DIRECTORIES}"
	"${_FUNC_NAMED_PARAMETERS_IGNORE_SOURCES}"
	"${EXTRA_IMMORTALC_ARGS}"
	)
