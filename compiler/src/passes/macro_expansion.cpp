 ...
 * \brief Expand macros - implementation
 *
 * \copyright Copyright 2022 The ImmortalThreads authors. All rights reserved.
 * \license MIT License. See accompanying file LICENSE.txt at
 * https://github.com/tinysystems/ImmortalThreads/blob/main/LICENSE.txt
 *
 * Copied from RewriteMacros.cpp
 */
...
static const Token &GetNextRawTok(const std::vector<Token> &RawTokens,
  ...
  while (!ReturnComment && RawTokens[CurTok].is(tok::comment))
    ++CurTok;
...
void MacroExpansionPass::register_matchers(
    clang::ast_matchers::MatchFinder &finder) {
  // register something so that onEndOfTranslationUnit is run
  finder.addMatcher(translationUnitDecl(), this);
}
/// Heavily (if not entirely) copied from RewriteMacrosInInput
/// The only difference is that we make changes using the functions of
/// ImmortalcPass, rather than working with RewriteBuffer.
void MacroExpansionPass::onEndOfTranslationUnit() {
  auto &CI = this->context_.get_current_CI();
  ...
  while (RawTok.isNot(tok::eof) || PPTok.isNot(tok::eof)) {
    ...
    if (RawTok.is(tok::hash) && RawTok.isAtStartOfLine()) {
      ...
      if (RawTokens[CurRawTok].is(tok::identifier)) {
        ...
        if (II->getName() == "warning") {
          // Comment out #warning.
          auto err = this->insert(SM, RawTok.getLocation(), "//");
          assert_no_error(err);
    ...
    if (RawOffs <= PPOffs) {
      ...
      {
        auto err = this->insert(SM, SM.getComposedLoc(main_file_id, RawOffs),
                                &" /*"[HasSpace]);
        assert_no_error(err);
      }
      unsigned EndPos;
      ...
      auto err = this->insert(SM, SM.getComposedLoc(main_file_id, EndPos), "*/",
                              false);
      assert_no_error(err);
      continue;
    }
    ...
    Expansion += ' ';
    auto err = this->insert(SM, SM.getComposedLoc(main_file_id, InsertPos),
                            Expansion, false);
    assert_no_error(err);
  }
}
