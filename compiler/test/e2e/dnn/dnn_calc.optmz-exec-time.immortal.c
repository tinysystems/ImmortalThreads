      ...
      for (; i < rows * params.stride[1];) {
                                        ...
                                        sizeof(uint16_t)))));
        {
          ...
          for (; j < cols * params.stride[2];) {
            ...
            if (!zero) {
              _WR_SELF_CONTEMPORANEOUS_2(fixed, w, w + *inter_ptr, fixed *,
                                         inter_ptr, inter_ptr + 1);
            }

            _WR(*dest_ptr, w);
            _WR_SELF_CONTEMPORANEOUS_3(fixed *, dest_ptr, dest_ptr + 1, fixed *,
                                       src_ptr, src_ptr + params.stride[2],
                                       uint16_t, j, j + params.stride[2]);
          }
        }
        _WR_SELF(uint16_t, i, i + params.stride[1]);
      }
    }
    _WR_SELF_CONTEMPORANEOUS_2(uint16_t, idx,
                               idx + filter->sparse.offsets[pos + 1], uint16_t,
                               pos, pos + 1);
  }
      ...
      for (; i < rows;) {
        {
          ...
          for (; j < cols;) {
            if (((sizeof((uint16_t[]){i, j}) / sizeof(uint16_t)) == 1)) {
              _WR_SELF(
                  fixed, *(inter->data + ((uint16_t[]){i, j})[0]),
...
_immortal_function(fc_sparse_mul, mat_t *src, mat_t *dest, mat_t *filter) {
...

  {
    ...
    for (; i < rows;) {
      ...
      offset = filter->sparse.offsets + col_idx;
...

      {
        ...
        for (; j < end - start;) {
          ...
          if (j == 0) {
            ...
            _WR_SELF(fixed, w, w + *dest_ptr);
            _WR(*dest_ptr, w);
          }
          _WR_SELF_CONTEMPORANEOUS_2(uint16_t *, offset, offset + 1, uint16_t,
                                     j, j + 1);
        }
      }
      _WR_SELF_CONTEMPORANEOUS_2(fixed *, dest_ptr, dest_ptr + 1, uint16_t, i,
                                 i + 1);
    }
  }
  _end(fc_sparse_mul);
    ...
    for (; i < rows;) {
      ...
      k = 0;
      {
        ...
        for (; j < cols;) {
          ...
          _WR(*dest_ptr, w);
          _WR_SELF_CONTEMPORANEOUS_3(uint16_t, k, k + 1, fixed *, src_ptr,
                                     src_ptr + 1, uint16_t, j, j + 1);
        }
      }
      _WR_SELF_CONTEMPORANEOUS_2(fixed *, dest_ptr, dest_ptr + 1, uint16_t, i,
                                 i + 1);
